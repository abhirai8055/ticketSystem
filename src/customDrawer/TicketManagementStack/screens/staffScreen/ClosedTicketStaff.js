...
//                 </View>
//                 <View style={styles.columnCell}>
//                   {renderStars(item.customerReview || 'No Review')}
//                 </View>
//                 <View style={styles.columnCell}>
//                   <Text
//                     style={[
...
export default function ClosedTicketStaff({ userUid: propUid, route }) {
  ...
  const [engineerOptions, setEngineerOptions] = useState(['All']);
  ...
  useEffect(() => {
    const fetchTickets = async () => {
      try {
        ...
        // Extract unique engineer names for dropdown
        const uniqueEngineers = [
          ...new Set(
            ticketsWithDetails
              .map(ticket => ticket.engineer?.displayName)
              .filter(engineer => engineer && engineer !== 'N/A'),
          ),
        ];
        setEngineerOptions(['All', ...uniqueEngineers]);
  ...
  const filteredTickets = tickets.filter(ticket => {
    ...
    const matchesEngineer =
      selectedEngineer === 'All' || ticket.engineer?.displayName === selectedEngineer;

    return matchesSearch && matchesCategory && matchesPriority && matchesEngineer;
  });
          ...
          <View style={styles.pickerContainer}>
            ...
            >
              {engineerOptions.map(engineer => (
                <Picker.Item key={engineer} label={engineer} value={engineer} />
              ))}
            </Picker>
          </View>
        </View>
