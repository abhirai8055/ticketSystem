...
//         const engineerIds = [...new Set(combinedTickets.map(ticket => ticket.engineerId))].filter(id => id);
//         const engineerNameMap = {};
//         if (engineerIds.length > 0) {
//           const usersQuery = query(usersRef, where('uid', 'in', engineerIds.slice(0, 10))); // Limit to 10 for performance
...
//           console.log(
//             `Ticket ID: ${ticket.ticketId}, Raw Priority: ${ticket.priority}, Normalized Priority: ${ticket.priority ? ticket.priority.toLowerCase() : 'N/A'}, Engineer ID: ${ticket.engineerId}, Engineer Name: ${engineerNameMap[ticket.engineerId] || 'N/A'}`,
//           );
//           return {
//             ...ticket,
//             categoryName: categoryMap[ticket.category] || 'N/A',
//             priority: ticket.priority ? ticket.priority.toLowerCase() : 'N/A',
//             engineerName: engineerNameMap[ticket.engineerId] || 'N/A', // Map engineerId to displayName
//           };
//         });
...
//                 </View>
//                 <View style={styles.columnCell}>
//                   <Text style={styles.cellText}>{item.engineerId || 'N/A'}</Text>
//                 </View>
//                 <View style={styles.columnCell}>
//                   <Text
//                     style={[
...
export default function AllTicketsStaff({ userUid: propUid, route }) {
    ...
    const fetchTickets = async () => {
      try {
        ...
        for (const ticket of ticketValue) {
          const enginner = await getUserById(ticket.engineerId);
          ticket.engineer = enginner;
        ...
        console.log('Engineer IDs:', engineerIds); // Debug: Log all engineerIds
        const engineerNameMap = {};
        if (engineerIds.length > 0) {
          const usersQuery = query(
            usersRef,
            where('uid', 'in', engineerIds), // Temporarily remove .slice(0, 10) for testing
          );
          const usersSnap = await getDocs(usersQuery);
          console.log('Users Data:', usersSnap.docs.map(doc => doc.data())); // Debug: Log user data
          usersSnap.forEach(doc => {
            engineerNameMap[doc.data().uid] = doc.data().displayName || 'N/A';
          });
        }
        ...
        const ticketsWithDetails = combinedTickets.map(ticket => {
          console.log(
            `Ticket ID: ${ticket.ticketId}, Raw Priority: ${
              ...
              ticket.priority ? ticket.priority.toLowerCase() : 'N/A'
            }, Engineer ID: ${ticket.engineerId}, Engineer Name: ${
              engineerNameMap[ticket.engineerId] || 'N/A'
            }`,
          );
          return {
            ...ticket,
  ...
  const filteredTickets = tickets.filter(ticket => {
    ...
    const matchesPriority =
      selectedPriority === 'All' ||
      (ticket.priority && ticket.priority === selectedPriority.toLowerCase()); // Match normalized values
          ...
          {tickets.length === 0 ? (
            ...
            paginatedTickets.map((item, index) => (
              ...
              >
                ...
                <View style={styles.columnCell}>
                  <Text style={styles.cellText}>{item.engineer?.displayName || 'N/A'}</Text>
                </View>
                <View style={styles.columnCell}>
                  <Text
                    style={[
